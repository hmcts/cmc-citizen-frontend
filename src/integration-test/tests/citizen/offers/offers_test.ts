...
Scenario('Claimant Accepted Offer  @citizen', { retries: 3 }, async (I: I) => {
...
Scenario('Defendant Countersigned Offer  @citizen', { retries: 3 }, async (I: I) => {
  userSteps.login(claimantEmail)
  offerSteps.acceptOfferFromDashboard(claimRef)
  I.click('Sign out')
...
Scenario('Claimant Rejected Offer @citizen ', { retries: 3 }, async (I: I) => {
  userSteps.login(claimantEmail)
...
Scenario('Settle Out Of Court E2E ', { retries: 3 }, async (I: I) => {
  userSteps.login(claimantEmail)
  offerSteps.acceptOfferFromDashboard(claimRef)
  I.click('Sign out')
