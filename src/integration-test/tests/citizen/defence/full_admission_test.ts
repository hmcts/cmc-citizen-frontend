...
if (process.env.FEATURE_ADMISSIONS === 'true') {
  ...
  Scenario('I can complete the journey when I fully admit all of the claim with immediate payment @citizen @admissions', { retries: 3 }, async (I: I) => {
  ...
  Scenario('I can complete the journey when I fully admit all of the claim with full payment by set date @citizen @admissions', { retries: 3 }, async (I: I) => {
    const claimData = await prepareClaim(I)
