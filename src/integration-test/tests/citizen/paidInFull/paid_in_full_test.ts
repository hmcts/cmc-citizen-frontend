...
Scenario('Check newly created claim is in my account dashboard with correct claim amount @citizen', async (I: I) => {
  ...
  const claimRef: string = await claimSteps.makeAClaimAndSubmit(email, PartyType.INDIVIDUAL, PartyType.INDIVIDUAL, false)

  // NO CCJ requested
  // CCJ requested within month
  // CCJ requested after month
  ...
  // click tell us you've been paid
  // i see title
  // i enter date
  // i click button
  // i see confirmation
