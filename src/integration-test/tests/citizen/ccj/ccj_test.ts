...
Scenario('Request judgment as a Company, pay by set date @nightly', { retries: 3 }, async (I: I, login) => {
  const email: string = userSteps.getClaimantEmail()
...
Scenario('Request judgment as a sole trader, pay immediately @nightly', { retries: 3 }, async (I: I, login) => {
  const email: string = userSteps.getClaimantEmail()
