...
import { company, individual, individualDefendant, organisation, soleTrader } from 'test/data/entity/party'
...
describe('ResponseModelConverter', () => {
  ...
  context('partial admission conversion', () => {
    ...
    it('should convert partial admission already paid', () => {
      ...
      expect(convertObjectLiteralToJSON(ResponseModelConverter.convert(responseDraft, new MediationDraft().deserialize({}), undefined, claim)))
        .to.deep.equal(convertObjectLiteralToJSON(responseData))
    })
  })
