...
const hearingExceptionalCircumstancesPage = paths_1.Paths.hearingExceptionalCircumstancesPage.evaluateUri({ externalId: externalId });
const hearingLocationPage = paths_1.Paths.hearingLocationPage.evaluateUri({ externalId: externalId });
const pagePath = paths_1.Paths.supportPage.evaluateUri({ externalId: externalId });
            ...
            context('when user authorised', () => {
                ...
                it('should render page when everything is fine', async () => {
                    ...
                    await request(app_1.app)
                        .get(pagePath)
                        .set('Cookie', `${cookieName}=ABC`)
                        .expect(res => chai_1.expect(res).to.be.successful.withText('Select any support you’d require for a court hearing'));
                });
            });
        });
        describe('on POST', () => {
                ...
                context('when form is invalid', async () => {
                    it('should render page when everything is fine', async () => {
                        ...
                        await request(app_1.app)
                            ...
                            .expect(res => chai_1.expect(res).to.be.successful.withText('Select any support you’d require for a court hearing', 'div class="error-summary"'));
                    });
                });
            });
            context('when user is authorised claimant and form is valid', () => {
                it('should redirect to hearing exceptional circumstances page', async () => {
                    const claim = Object.assign(Object.assign({}, claimWithDQ), claimStoreServiceMock.sampleDefendantResponseObj);
                    ...
                    await request(app_1.app)
                        ...
                        .set('Cookie', `${cookieName}=ABC`)
                        .send(validFormData)
                        .expect(res => chai_1.expect(res).to.be.redirect.toLocation(hearingExceptionalCircumstancesPage));
                });
            });
            context('when user is authorised defendant and form is valid', () => {
                it('should redirect to hearing location page', async () => {
                    ...
                    await request(app_1.app)
                        ...
                        .set('Cookie', `${cookieName}=ABC`)
                        .send(validFormData)
                        .expect(res => chai_1.expect(res).to.be.redirect.toLocation(hearingLocationPage));
                });
            });
        });
    }
