...
const datesPage = paths_1.Paths.hearingDatesPage.evaluateUri({ externalId: externalId });
const pagePath = paths_1.Paths.otherWitnessesPage.evaluateUri({ externalId: externalId });
            ...
            context('when user authorised', () => {
                ...
                it('should render page when everything is fine', async () => {
                    ...
                    await request(app_1.app)
                        .get(pagePath)
                        .set('Cookie', `${cookieName}=ABC`)
                        .expect(res => chai_1.expect(res).to.be.successful.withText('Do you want other people to give evidence?'));
                });
            });
        });
        describe('on POST', () => {
            const validFormData = { otherWitnesses: yesNoOption_1.YesNoOption.NO.option };
            const invalidFormData = { otherWitnesses: yesNoOption_1.YesNoOption.YES.option };
            const method = 'post';
            authorization_check_1.checkAuthorizationGuards(app_1.app, method, pagePath);
            checkAccessGuard(app_1.app, method);
            context('when defendant authorised', () => {
                ...
                context('when form is valid', async () => {
                    ...
                    it('should redirect to dates page', async () => {
                        ...
                        await request(app_1.app)
                            ...
                            .set('Cookie', `${cookieName}=ABC`)
                            .send(validFormData)
                            .expect(res => chai_1.expect(res).to.be.redirect.toLocation(datesPage));
                    });
                });
                context('when form is invalid', async () => {
                    it('should render page when everything is fine', async () => {
                        ...
                        await request(app_1.app)
                            ...
                            .expect(res => chai_1.expect(res).to.be.successful.withText('Do you want other people to give evidence?', 'div class="error-summary"'));
                    });
                });
            });
        });
