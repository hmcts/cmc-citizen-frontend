...
const expertReportsPage = paths_1.Paths.expertReportsPage.evaluateUri({ externalId });
const pagePath = paths_1.Paths.expertPage.evaluateUri({ externalId });
            ...
            context('when user authorised', () => {
                ...
                it('should render page when everything is fine', async () => {
                    ...
                    await request(app_1.app)
                        .get(pagePath)
                        .set('Cookie', `${cookieName}=ABC`)
                        .expect(res => chai_1.expect(res).to.be.successful.withText('Using an expert', 'It’s rare for a judge to allow you to use an expert in a small claim. Most small claims don’t need an expert.'));
                });
            });
        });
        describe('on POST', () => {
                ...
                context('when form is valid', async () => {
                    ...
                    it('should redirect to expert reports page when expert is needed', async () => {
                        ...
                        await request(app_1.app)
                            ...
                            .set('Cookie', `${cookieName}=ABC`)
                            .send(expertRequiredFormData)
                            .expect(res => chai_1.expect(res).to.be.redirect.toLocation(expertReportsPage));
                    });
                });
            });
        });
