...
describe('MonthlyIncomeSource', () => {
  describe('fromObject', () => {
    it('should return undefined when undefined provided as object parameter', () => {
      expect(MonthlyIncomeSource.fromObject('name', undefined)).to.eql(undefined)
    })

    it('should return undefined when no object parameter provided', () => {
      expect(MonthlyIncomeSource.fromObject('name')).to.deep.equal(undefined)
    })
    ...
    it('should return a new instance initialised with set fields from object parameter provided', () => {
      expect(MonthlyIncomeSource.fromObject('name', SAMPLE_MONTHLY_INCOME_SOURCE_FROM_OBJECT)).to.deep.equal(
    ...
    it('should return a new instance initialised with name set to undefined', () => {
      expect(MonthlyIncomeSource.fromObject(undefined)).to.eql(undefined, undefined)
    })
  })

  describe('deserialize', () => {
    it('should return instance initialised with defaults when undefined provided', () => {
      expect(new MonthlyIncomeSource('name').deserialize(undefined)).to.deep.equal(new MonthlyIncomeSource('name'))
    })

    it('should return instance initialised with set fields from object provided', () => {
      expect(new MonthlyIncomeSource('name').deserialize(SAMPLE_MONTHLY_INCOME_SOURCE_DESERIALIZE)).to.deep.equal(
        new MonthlyIncomeSource(
          'name',
          100,
          IncomeExpenseSchedule.MONTH
  ...
  describe('validation', () => {
    ...
    describe('when not successful', () => {
      it('should return errors when all expect `name` are undefined', () => {
        const errors = validator.validateSync(new MonthlyIncomeSource('Source name', undefined))
      ...
      it('should return an error when `schedule` is undefined', () => {
        const errors = validator.validateSync(new MonthlyIncomeSource('Source name', 100, undefined))
