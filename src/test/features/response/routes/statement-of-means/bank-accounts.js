...
const pagePath = paths_1.StatementOfMeansPaths.bankAccountsPage.evaluateUri({ externalId: claimStoreServiceMock.sampleClaimObj.externalId });
            ...
            context('when response and CCJ not submitted', () => {
                ...
                it('should render page when everything is fine', async () => {
                    ...
                    await request(app_1.app)
                        .get(pagePath)
                        .set('Cookie', `${cookieName}=ABC`)
                        .expect(res => chai_1.expect(res).to.be.successful.withText('List your bank and savings accounts'));
                });
            });
        });
    });
        ...
        describe('for authorized user', () => {
            ...
            describe('save and continue', () => {
                it('should update draft store and redirect', async () => {
                    ...
                    await request(app_1.app)
                        .post(pagePath)
                        .send({ rows: [{ typeOfAccount: bankAccountType_1.BankAccountType.ISA.value, joint: false, balance: 10 }] })
                        .set('Cookie', `${cookieName}=ABC`)
                        .expect(res => chai_1.expect(res).to.be.redirect
                        .toLocation(paths_1.StatementOfMeansPaths.disabilityPage.evaluateUri({ externalId: claimStoreServiceMock.sampleClaimObj.externalId })));
                });
            });
