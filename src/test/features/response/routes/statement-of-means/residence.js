...
const residencePage = paths_1.StatementOfMeansPaths.residencePage.evaluateUri({
...
describe('Statement of means', () => {
    describe('Residence page', () => {
        ...
        describe('on GET', () => {
            authorization_check_1.checkAuthorizationGuards(app_1.app, 'get', residencePage);
            context('when user authorised', () => {
                beforeEach(() => {
                    idamServiceMock.resolveRetrieveUserFor(claimStoreServiceMock.sampleClaimObj.defendantId, 'citizen');
                });
                alreadyPaidInFullGuard_1.verifyRedirectForGetWhenAlreadyPaidInFull(residencePage);
                ...
                it('should return successful response when claim is retrieved', async () => {
                    ...
                    await request(app_1.app)
                        .get(residencePage)
                        .set('Cookie', `${cookieName}=ABC`)
                        .expect(res => chai_1.expect(res).to.be.successful.withText('Where do you live?'));
                });
            });
        });
        describe('on POST', () => {
            const validFormData = {
                type: residenceType_1.ResidenceType.OWN_HOME.value
            };
            authorization_check_1.checkAuthorizationGuards(app_1.app, 'get', residencePage);
            context('when user authorised', () => {
                beforeEach(() => {
                ...
                alreadyPaidInFullGuard_1.verifyRedirectForPostWhenAlreadyPaidInFull(residencePage);
                it('should return error page when unable to retrieve claim', async () => {
                    claimStoreServiceMock.rejectRetrieveClaimByExternalId('Error');
                    await request(app_1.app)
                        .post(residencePage)
                ...
                it('should redirect to dependants page when all is fine and form is valid', async () => {
                    ...
                    await request(app_1.app)
                        ...
                        .toLocation(paths_1.StatementOfMeansPaths.partnerPage
                ...
                it('should trigger validation when all is fine and form is invalid', async () => {
                    ...
                    await request(app_1.app)
                        ...
                        .set('Cookie', `${cookieName}=ABC`)
                        .send({})
                        .expect(res => chai_1.expect(res).to.be.successful.withText(validationErrors_1.ValidationErrors.SELECT_AN_OPTION));
                });
            });
        });
    });
