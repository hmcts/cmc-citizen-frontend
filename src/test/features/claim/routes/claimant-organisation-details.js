...
const heading = 'Enter organisation details';
const input = {
    name: 'ABC Ltd',
    type: 'organisation',
...
describe('claimant as organisation details page', () => {
    ...
    describe('on GET', () => {
        authorization_check_1.checkAuthorizationGuards(app_1.app, 'get', paths_1.Paths.claimantOrganisationDetailsPage.uri);
        eligibility_check_1.checkEligibilityGuards(app_1.app, 'get', paths_1.Paths.claimantOrganisationDetailsPage.uri);
        it('should render page when everything is fine', async () => {
            idamServiceMock.resolveRetrieveUserFor('1', 'citizen');
            draftStoreServiceMock.resolveFind('claim');
            await request(app_1.app)
                .get(paths_1.Paths.claimantOrganisationDetailsPage.uri)
                .set('Cookie', `${cookieName}=ABC`)
                .expect(res => chai_1.expect(res).to.be.successful.withText(heading));
        });
    });
    describe('on POST', () => {
        authorization_check_1.checkAuthorizationGuards(app_1.app, 'post', paths_1.Paths.claimantOrganisationDetailsPage.uri);
        eligibility_check_1.checkEligibilityGuards(app_1.app, 'post', paths_1.Paths.claimantOrganisationDetailsPage.uri);
        describe('for authorized user', () => {
            ...
            it('should render page with error when claimant name is invalid', async () => {
                ...
                await request(app_1.app)
                    .post(paths_1.Paths.claimantOrganisationDetailsPage.uri)
                    .set('Cookie', `${cookieName}=ABC`)
            ...
            it('should redirect to phone page when everything is fine and not including contact person', async () => {
                const noContactPersonInput = Object.assign(Object.assign({}, input), { contactPerson: undefined });
                draftStoreServiceMock.resolveFind('claim');
                draftStoreServiceMock.resolveUpdate();
                await request(app_1.app)
                    .post(paths_1.Paths.claimantOrganisationDetailsPage.uri)
