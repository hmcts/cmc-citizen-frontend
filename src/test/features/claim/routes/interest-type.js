...
const pagePath = paths_1.Paths.interestTypePage.uri;
        ...
        describe('for authorized user', () => {
            ...
            it('should return 500 and render error page when form is valid and cannot save draft', async () => {
                ...
                await request(app_1.app)
                    .post(pagePath)
                    .set('Cookie', `${cookieName}=ABC`)
                    .send({ option: interestType_1.InterestTypeOption.SAME_RATE })
            ...
            it('should redirect to interest rate page when form is valid, same rate is selected and everything is fine', async () => {
                ...
                await request(app_1.app)
                    ...
                    .set('Cookie', `${cookieName}=ABC`)
                    .send({ option: interestType_1.InterestTypeOption.SAME_RATE })
                    .expect(res => chai_1.expect(res).to.be.redirect.toLocation(paths_1.Paths.interestRatePage.uri));
            });
            it('should redirect to interest total page when form is valid, breakdown is selected and everything is fine', async () => {
                ...
                await request(app_1.app)
                    .post(pagePath)
                    .set('Cookie', `${cookieName}=ABC`)
                    .send({ option: interestType_1.InterestTypeOption.BREAKDOWN })
                    .expect(res => chai_1.expect(res).to.be.redirect.toLocation(paths_1.Paths.interestTotalPage.uri));
            });
        });
    });
});
