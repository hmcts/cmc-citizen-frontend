...
describe('claimant as company details page', () => {
    ...
    describe('on GET', () => {
        authorization_check_1.checkAuthorizationGuards(app_1.app, 'get', paths_1.Paths.claimantCompanyDetailsPage.uri);
        eligibility_check_1.checkEligibilityGuards(app_1.app, 'get', paths_1.Paths.claimantCompanyDetailsPage.uri);
        it('should render page when everything is fine', async () => {
            idamServiceMock.resolveRetrieveUserFor('1', 'citizen');
            draftStoreServiceMock.resolveFind('claim');
            await request(app_1.app)
                .get(paths_1.Paths.claimantCompanyDetailsPage.uri)
                .set('Cookie', `${cookieName}=ABC`)
                .expect(res => chai_1.expect(res).to.be.successful.withText(heading));
        });
    });
    describe('on POST', () => {
        authorization_check_1.checkAuthorizationGuards(app_1.app, 'post', paths_1.Paths.claimantCompanyDetailsPage.uri);
        eligibility_check_1.checkEligibilityGuards(app_1.app, 'post', paths_1.Paths.claimantCompanyDetailsPage.uri);
        describe('for authorized user', () => {
            ...
            it('should render page with error when claimant name is invalid', async () => {
                ...
                await request(app_1.app)
                    .post(paths_1.Paths.claimantCompanyDetailsPage.uri)
                    .set('Cookie', `${cookieName}=ABC`)
