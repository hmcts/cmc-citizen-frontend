...
const pagePath = paths_1.Paths.intentionToProceedPage.evaluateUri({ externalId: externalId });
        ...
        describe('for authorized user', () => {
            ...
            it('should return 500 and render error page when form is valid and cannot save draft', async () => {
                ...
                await request(app_1.app)
                    .post(pagePath)
                    .set('Cookie', `${cookieName}=ABC`)
                    .send({ proceed: yesNoOption_1.YesNoOption.YES })
                    .expect(res => chai_1.expect(res).to.be.serverError.withText('Error'));
            });
            ...
            it('should redirect to task list page when No is selected and everything is fine', async () => {
                ...
                await request(app_1.app)
                    .post(pagePath)
                    .set('Cookie', `${cookieName}=ABC`)
                    .send({ proceed: yesNoOption_1.YesNoOption.NO })
                    .expect(res => chai_1.expect(res).to.be.redirect.toLocation(paths_1.Paths.taskListPage.evaluateUri({ externalId: externalId })));
            });
        });
    });
