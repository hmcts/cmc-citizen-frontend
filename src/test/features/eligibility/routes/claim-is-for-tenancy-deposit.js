...
const pagePath = paths_1.Paths.claimIsForTenancyDepositPage.uri;
const pageRedirect = paths_1.Paths.governmentDepartmentPage.uri;
...
describe('Claim eligibility: is claim for tenancy deposit page', () => {
    ...
    context('on POST', () => {
        ...
        it('should redirect to eligible page when form is valid and everything is fine', async () => {
            await request(app_1.app)
                .post(pagePath)
                .send({ claimIsForTenancyDeposit: yesNoOption_1.YesNoOption.NO.option })
                .expect(res => chai_1.expect(res).to.be.redirect.toLocation(pageRedirect));
        });
        it('should redirect to not eligible page when form is valid and not eligible option selected', async () => {
            await request(app_1.app)
                .post(pagePath)
                .send({ claimIsForTenancyDeposit: yesNoOption_1.YesNoOption.YES.option })
                .expect(res => chai_1.expect(res).to.be.redirect.toLocation(`${paths_1.Paths.notEligiblePage.uri}?reason=${notEligibleReason_1.NotEligibleReason.CLAIM_IS_FOR_TENANCY_DEPOSIT}`));
        });
    });
});
