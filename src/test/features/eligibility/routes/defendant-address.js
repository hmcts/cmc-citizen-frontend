...
const pagePath = paths_1.Paths.defendantAddressPage.uri;
const pageRedirect = paths_1.Paths.claimTypePage.uri;
const expectedTextOnPage = 'Does the person or organisation you’re claiming against have a postal address in England or Wales?';
const notEligibleReason = notEligibleReason_1.NotEligibleReason.DEFENDANT_ADDRESS;
describe('Claim eligibility: defendant address page', () => {
    ...
    context('on POST', () => {
        ...
        it('should redirect to over 18 page when form is valid and everything is fine', async () => {
            await request(app_1.app)
                .post(pagePath)
                .send({ defendantAddress: yesNoOption_1.YesNoOption.YES.option })
                .expect(res => chai_1.expect(res).to.be.redirect.toLocation(pageRedirect));
        });
        it('should redirect to not eligible page when form is valid and not eligible option selected', async () => {
            await request(app_1.app)
                .post(pagePath)
                .send({ defendantAddress: yesNoOption_1.YesNoOption.NO.option })
