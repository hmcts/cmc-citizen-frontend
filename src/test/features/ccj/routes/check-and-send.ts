...
const cnsPage = CCJPaths.checkAndSendPage.uri.replace(':externalId', externalId)
const ccjConfirmationPage = CCJPaths.ccjRequestedConfirmationPage.uri.replace(':externalId', externalId)
      ...
      context('when form is valid', async () => {
        it('should redirect to confirmation page', async () => {
          ...
          await request(app)
            ...
            .set('Cookie', `${cookieName}=ABC`)
            .send(validFormData)
            .expect(res => expect(res).to.be.redirect.toLocation(ccjConfirmationPage))
        })
      })
