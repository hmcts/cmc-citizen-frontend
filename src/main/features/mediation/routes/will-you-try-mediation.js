...
function renderView(form, res) {
    ...
    res.render(paths_1.Paths.willYouTryMediation.associatedView, {
        form: form,
        hint: hint,
...
exports.default = express.Router()
    .get(paths_1.Paths.willYouTryMediation.uri, async (req, res, next) => {
    const draft = res.locals.mediationDraft;
    renderView(new form_1.Form(draft.document.willYouTryMediation), res);
})
    .post(paths_1.Paths.willYouTryMediation.uri, formValidator_1.FormValidator.requestHandler(freeMediation_1.FreeMediation), errorHandling_1.ErrorHandling.apply(async (req, res, next) => {
    const form = req.body;
    if (form.hasErrors()) {
        renderView(form, res);
    }
