...
export default express.Router()
  .get(Paths.dashboardPage.uri, ErrorHandling.apply(async (req: express.Request, res: express.Response, next: express.NextFunction): Promise<void> => {
    ...
    const claimDraftSaved: boolean = user.claimDraft.document && user.claimDraft.document.lastUpdateTimestamp !== undefined
    const responseDraftSaved = user.responseDraft && user.responseDraft.document && user.responseDraft.document.lastUpdateTimestamp !== undefined // TODO: apply response draft middleware
