...
ValidationErrors.NAME_REQUIRED = 'Enter other expense source';
ValidationErrors.AMOUNT_REQUIRED = (name) => `Enter how much you pay for ${name ? name : monthlyExpenseType_1.MonthlyExpenseType.OTHER.displayValue}`;
ValidationErrors.AMOUNT_INVALID_DECIMALS = (name) => `Enter a valid ${name ? name : monthlyExpenseType_1.MonthlyExpenseType.OTHER.displayValue} amount, maximum two decimal places`;
ValidationErrors.AMOUNT_NON_NEGATIVE_NUMBER_REQUIRED = (name) => `Enter a valid ${name ? name : monthlyExpenseType_1.MonthlyExpenseType.OTHER.displayValue} amount, maximum two decimal places`;
ValidationErrors.SCHEDULE_SELECT_AN_OPTION = (name) => `Select how often you pay for ${name ? name : monthlyExpenseType_1.MonthlyExpenseType.OTHER.displayValue}`;
...
class ExpenseSource {
    ...
    static fromObject(name, value) {
        ...
        return new ExpenseSource(name, numericUtils_1.toNumberOrUndefined(value.amount), incomeExpenseSchedule_1.IncomeExpenseSchedule.of(value.schedule));
    }
    deserialize(input) {
        if (input) {
...
], ExpenseSource.prototype, "name", void 0);
__decorate([
...
], ExpenseSource.prototype, "amount", void 0);
__decorate([
...
], ExpenseSource.prototype, "schedule", void 0);
exports.ExpenseSource = ExpenseSource;
